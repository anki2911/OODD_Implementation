f1 = readtable('./Dataset/OOD_StratSwitch_SafeAction.csv');

Episode1 = table2array(f1(:,"Episode"));
Timestep1 = table2array(f1(:,"Step_no"));
%Freq1 = table2array(f1(:,"Freq"));

f2 = readtable('./Dataset/OOD_StratSwitch_wo_StratSel.csv');
...
Values = {'Known Red Strategy)','(Unknown Red Strategy)'};
...
xlabel('\bf{Strategy Switch}','fontsize',30,'FontWeight','bold','Interpreter','latex');
set(gca, 'XTick', [1, 2, 3, 4]); 
set(gca, 'XTickLabel', {'\bf{\boldmath$Meander \rightarrow B\_line$}','\bf{\boldmath$Meander \rightarrow Unknown$}'},'TickLabelInterpreter','latex','fontweight','bold','fontsize',30);
...
title(['\bf{Strategy switching to known/unknown strategy }'],'FontSize',30,'FontWeight','bold','Interpreter','latex');

box_vars = findobj(gca,'Tag','Box');
set(findobj(gca,'Type','text'),'FontSize',30,'FontWeight','bold','interpreter','latex','VerticalAlignment','middle') % to set Xaxis
